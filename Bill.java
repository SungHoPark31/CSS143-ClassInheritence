 ...
 *
 */
public class Bill implements Cloneable, Serializable, Comparable
{
	/**
	 * 
	 */
	...
	@Override
	/**
	 * Precondition: Object is not null when passing in the parameter
	 * 
	 * Postcondition: Lets say we have a.compareTo(b)
	 * Returns a negative number if a is before b.
	 * Returns positive number if a is after b.
	...
	{
		Bill o = (Bill)other;
		Money m1 = this.getAmount();
		Money m2 = o.getAmount();
		
		Date d1 = this.getDueDate();
		Date d2 = o.getDueDate();
		
		//Look at the money first then look at the date. 
		if(m1.compareTo(m2) == 0)
		{
			//Look at date, then look at the String
			if(d1.compareTo(d2) == 0)
			{
				//Check Originator
				if(this.getOriginator().equals(o.getOriginator()))
				{
					return 0;
				}
				else if(this.getOriginator().compareTo(o.getOriginator()) < 0)
				{
					return -1;
				}
				else if(this.getOriginator().compareTo(o.getOriginator()) > 0)
				...
				//If originator is not being compared, then return 0.
				return 0;
			}
			...
			//If date is not being compared, return 0.
			return 0;
		}
		//Move along with comparing money.
	...
	@Override
	/**
	 * Precondition: What you are cloning has values.
	 * 
	 * Postconditions: Copies the instance variables in the class.
	 */
	public Object clone()
	{
		//Try to clone it and have an exception just in case.
		try
		{
		...
		{
			//This shouldn't happen but it is here to keep the compiler happy
			return null;
		}
	}
}
